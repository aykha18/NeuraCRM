#!/usr/bin/env python3
"""
Playwright automation script for TC_CONTACT_005: Delete Contact
"""
...
def test_delete_contact(page: Page):
    ...
    if len(contact_rows) == 0:
        raise AssertionError("No contacts found to delete")
    ...
    print(f"Attempting to delete contact: {contact_name}")
    ...
    for selector in delete_selectors:
        try:
            delete_button = first_contact.locator(selector).first
    ...
    # Check if the contact no longer appears in the list
    if not page.locator(f'text="{contact_name}"').is_visible():
        print("Contact no longer appears in the list")
        success_found = True
    
    # Check if the contact count decreased
    current_contact_rows = page.locator('[data-testid="contact-row"], tr:has(td), .contact-item, .table-row').all()
    if len(current_contact_rows) < len(contact_rows):
        print("Contact count decreased")
        success_found = True
    
    # Take screenshot for debugging
    page.screenshot(path="test-results/contacts_delete.png")
    
    if success_found:
        return True
    else:
        raise AssertionError("Contact deletion success not confirmed")
...
if __name__ == "__main__":
    ...
    with sync_playwright() as p:
        ...
        try:
            result = test_delete_contact(page)
            print("PASSED: TC_CONTACT_005: Delete Contact")
        except Exception as e:
            print(f"FAILED: TC_CONTACT_005: Delete Contact - {e}")
            page.screenshot(path="test-results/contacts_delete_failure.png")
        finally:
            browser.close()
