...
def get_kanban_board_optimized(
    ...
    try:
        ...
        query = db.query(Deal, User.name.label("owner_name"), Contact.name.label("contact_name")).            join(User, Deal.owner_id == User.id, isouter=True).            join(Contact, Deal.contact_id == Contact.id, isouter=True).            filter(Deal.organization_id == org_id)
