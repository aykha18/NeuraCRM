...
def get_tables(conn, db_name):
    ...
    try:
        ...
        print(f"[INFO] {db_name}: Found {len(tables)} tables")
        return set(tables)
    except Exception as e:
        print(f"[ERROR] Error getting tables from {db_name}: {e}")
        return set()
...
def get_table_columns(conn, table_name, db_name):
    ...
    try:
        ...
        print(f"[ERROR] Error getting columns for {table_name} in {db_name}: {e}")
        return {}
